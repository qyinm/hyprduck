...
`crates/hyprduck-cli` owns the human CLI, eval/TUI helpers, and the MCP server that external agents use for cited reads and controlled mutations.
...
- Prefer the MCP-focused `hyprduck-cli` cargo filters for changes in `src/mcp.rs`, `src/mcp/`, or `tests/mcp_server.rs`.
