...
#[cfg(test)]
#[derive(Debug, Clone)]
struct SourceGraphCompactionResult {
    raw_snapshot: BrainRepoSnapshot,
    canonical_snapshot: BrainRepoSnapshot,
    report: SourceGraphCompactionReport,
}
...
struct SourceGraphChunkBatchPlan {
    batches: Vec<Vec<crate::source_index::SourceChunk>>,
    discovered_batch_count: usize,
    skipped_batch_count: usize,
}

fn source_graph_chunk_batch_plan(
    manifest: &SourceArtifactManifest,
    markdown: &str,
...
fn write_source_chunk_run_artifact(
    artifact_root: &Path,
    run_id: &str,
    workspace_id: &str,
    source_id: &str,
...
fn write_graph_candidate_batch_artifact(
    artifact_root: &Path,
    source_run_id: &str,
    chunk_run_id: &str,
    _workspace_id: &str,
...
fn source_chunk_evidence_refs(batch: &[crate::source_index::SourceChunk]) -> Vec<EvidenceRef> {
    batch
        .iter()
...
fn parse_and_validate_source_graph_response(
    response: &str,
    workspace_id: &str,
...
#[cfg(test)]
fn merge_source_graph_snapshots(
    workspace_id: &str,
    baseline: &BrainRepoSnapshot,
    source_id: &str,
...
fn merge_raw_source_graph_snapshots(
    workspace_id: &str,
    baseline: &BrainRepoSnapshot,
    source_id: &str,
    snapshots: Vec<BrainRepoSnapshot>,
...
fn compact_source_graph_snapshot(
    workspace_id: &str,
    baseline: &BrainRepoSnapshot,
    source_id: &str,
...
fn strip_source_of_relations(snapshot: &mut BrainRepoSnapshot) -> usize {
...
fn normalize_concept_label(value: &str) -> String {
