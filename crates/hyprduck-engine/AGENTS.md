...
`crates/hyprduck-engine` owns parsing, provider execution, durable knowledge storage, graph/wiki materialization, context packs, and source-backed answer behavior.
...
- Prefer a focused `hyprduck-engine` cargo filter for local domain changes before running the broader core test alias.
- If an engine change alters MCP-visible behavior, also verify the relevant `hyprduck-cli` MCP tests.
