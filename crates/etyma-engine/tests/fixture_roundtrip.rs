...
fn fixture_path(name: &str) -> PathBuf {
    repo_root()
        .join("crates/etyma-engine/tests/fixtures")
        .join(name)
}
...
fn run_parse(fixture: &str, format: &str) -> ParseResponseData {
    ...
    let mut child = Command::new(env!("CARGO_BIN_EXE_etyma-engine"))
        .env("ETYMA_CONFIG_DIR", config_dir.path())
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
...
fn assert_source_artifacts(result: &ParseResponseData, format: &str) {
    ...
    assert_eq!(
        manifest.status,
        etyma_engine_types::IngestStatus::Ingested
    );
...
fn readiness_reports_runtime_config_and_dependencies() {
    ...
    assert_eq!(
        response.command,
        etyma_engine_types::EngineCommand::CheckReadiness
    );
