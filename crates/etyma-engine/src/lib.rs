...
const CANONICAL_STATE_STORE_SQLITE_GRAPHQLITE: &str = "etyma.sqlite+graphqlite";
const PROVIDER_GRAPH_AGENT_ID: &str = "etyma-provider-graph-agent";
const MCP_WRITE_AGENT_ID: &str = "etyma-mcp-write-agent";
...
pub(crate) fn resolve_brain_workspace_root(scope: &BrainReadScope) -> Result<PathBuf> {
    ...
    } else if let Some(output_root) = std::env::var_os("ETYMA_OUTPUT_DIR")
        .or_else(|| std::env::var_os("HYPRDUCK_OUTPUT_DIR"))
    {
        // HYPRDUCK_OUTPUT_DIR is a temporary alias during the Etyma rename.
        ...
        resolve_workspace_root_from_base(
            &resolve_application_support_product_dir(&application_support_root),
            &scope.workspace_id,
        )?
    } else {
        resolve_workspace_root_from_base(
            &resolve_application_support_product_dir(&std::env::temp_dir()),
            &scope.workspace_id,
        )?
    };
...
/// Prefer `Etyma/`; fall back to legacy `HyprDuck/` when that is the only existing data root.
fn resolve_application_support_product_dir(base: &Path) -> PathBuf {
    let etyma = base.join("Etyma");
    let legacy = base.join("HyprDuck");
    if etyma.exists() || !legacy.exists() {
        etyma
    } else {
        legacy
    }
