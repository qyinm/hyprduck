...
fn seed_agent_patch_workspace() -> (tempfile::TempDir, PathBuf, BrainReadScope) {
    ...
    let output_root = temp.path().join("Etyma");
...
fn agent_graph_patch() -> etyma_engine_types::GraphPatch {
    etyma_engine_types::GraphPatch {
        schema_version: etyma_engine_types::GRAPH_PATCH_SCHEMA_VERSION.into(),
        source_ids: vec!["source-agent".into()],
        evidence_refs: vec!["ev-agent-1".into()],
        nodes: vec![etyma_engine_types::GraphPatchNode {
        ...
        relations: vec![etyma_engine_types::GraphPatchRelation {
        ...
        claims: vec![etyma_engine_types::GraphPatchClaim {
        ...
        wiki_pages: vec![etyma_engine_types::GraphPatchWikiPage {
...
fn agent_graph_patch_applies_evidence_backed_nodes_and_relations() {
    ...
    let response = handle_apply_graph_patch(etyma_engine_types::ApplyGraphPatchRequest {
        scope: scope.clone(),
    ...
    let node = handle_read_node(etyma_engine_types::ReadNodeRequest {
        scope: scope.clone(),
        node_id: "concept-agent-graph-patch".into(),
    })
    ...
    let snapshot = handle_read_graph_snapshot(etyma_engine_types::ReadGraphSnapshotRequest {
        scope,
...
fn agent_graph_patch_rejects_records_without_direct_evidence() {
    ...
    let error = handle_apply_graph_patch(etyma_engine_types::ApplyGraphPatchRequest {
        scope,
        agent_id: Some("codex".into()),
        graph_patch: patch,
    })
...
fn agent_graph_patch_preserves_existing_record_refs_when_updating_in_scope_node() {
    ...
    handle_apply_graph_patch(etyma_engine_types::ApplyGraphPatchRequest {
        scope: scope.clone(),
        agent_id: Some("codex".into()),
        graph_patch: patch,
    })
...
fn read_graph_snapshot_includes_materialization_report_counts() {
    ...
    let response = handle_read_graph_snapshot(etyma_engine_types::ReadGraphSnapshotRequest {
    ...
    let search_response = handle_search_brain(etyma_engine_types::SearchBrainRequest {
        scope: BrainReadScope {
            workspace_id: DEFAULT_WORKSPACE_ID.into(),
            root_dir: Some(output_root.display().to_string()),
        },
...
fn workspace_delete_materialized_node_without_source_rows_returns_empty_project() {
    let _guard = TEST_ENV_LOCK.lock().expect("env lock");
    let temp = tempfile::tempdir().expect("temp dir");
    let store_path = temp.path().join("Etyma").join("knowledge.sqlite3");
    let output_root = temp.path().join("Etyma");
    let workspace_root = output_root.join(DEFAULT_WORKSPACE_ID);
    let mut snapshot = empty_replayed_brain_snapshot(DEFAULT_WORKSPACE_ID);
    snapshot.generated_at = 10;
    ...
    let previous_store = std::env::var_os("ETYMA_PROJECT_STORE");
    let previous_output_root = std::env::var_os("ETYMA_OUTPUT_DIR");
    std::env::set_var("ETYMA_PROJECT_STORE", &store_path);
    std::env::set_var("ETYMA_OUTPUT_DIR", &output_root);
    ...
    match previous_output_root {
        Some(value) => std::env::set_var("ETYMA_OUTPUT_DIR", value),
        None => std::env::remove_var("ETYMA_OUTPUT_DIR"),
    }
...
fn deleting_source_replays_provider_graph_for_remaining_sources() {
    ...
    let provider_event = BrainEvent {
        ...
        actor: BrainActor {
            ...
            actor_id: "etyma-provider-graph-agent:full-workspace-rebuild".into(),
        },
    ...
    let previous_store = std::env::var_os("ETYMA_PROJECT_STORE");
    let previous_output = std::env::var_os("ETYMA_OUTPUT_DIR");
    std::env::set_var("ETYMA_PROJECT_STORE", &store_path);
    std::env::set_var("ETYMA_OUTPUT_DIR", temp.path());
...
fn workspace_linking_artifacts_drop_after_source_deletion_breaks_cross_source_refs() {
    ...
    let linking_event = BrainEvent {
        ...
        actor: BrainActor {
            actor_type: BrainActorType::Agent,
            actor_id: "etyma-provider-workspace-linking-agent:test".into(),
        },
        source_refs: vec![
            active_source.source_id.clone(),
            deleted_source.source_id.clone(),
...
fn provider_test_event(input: ProviderTestEventInput<'_>) -> BrainEvent {
    BrainEvent {
        ...
        actor: BrainActor {
            actor_type: BrainActorType::Agent,
            actor_id: "etyma-provider-graph-agent:test".into(),
        },
        source_refs: input
            .sources
            .iter()
