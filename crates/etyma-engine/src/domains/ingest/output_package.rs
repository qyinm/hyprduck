...
fn output_root_candidates(
    output: &etyma_engine_types::ParseOutputTarget,
) -> Result<Vec<PathBuf>> {
    ...
    if let Some(override_root) = std::env::var_os("ETYMA_OUTPUT_DIR") {
        candidates.push(PathBuf::from(override_root));
    } else {
        if let Some(application_support_root) = dirs::data_local_dir() {
            candidates.push(application_support_root.join("Etyma"));
        }
        candidates.push(std::env::temp_dir().join("Etyma"));
    }
...
struct SourceImportArtifacts {
    source_pack: etyma_engine_types::SourcePackV0,
    evidence_index: etyma_engine_types::EvidenceIndexV1,
}
    ...
    fn build(self) -> SourceImportArtifacts {
        let source_pack = etyma_engine_types::SourcePackV0 {
            ...
            pages: self
                ...
                .pages
                .iter()
                .map(|page| etyma_engine_types::SourcePackPageV0 {
                    page: page.index + 1,
...
fn source_pack_warnings(
    manifest: &SourceArtifactManifest,
) -> Vec<etyma_engine_types::SourcePackWarningV0> {
    manifest
        .pages
        .iter()
        .filter_map(|page| {
            page.error_message
                .as_ref()
                .map(|message| etyma_engine_types::SourcePackWarningV0 {
...
fn evidence_index_item(
    ...
    page: &PageArtifact,
    content_hash: &str,
) -> Option<etyma_engine_types::EvidenceIndexItemV1> {
    let raw_text = page
        .markdown_path
        .as_ref()
    ...
    let quoted_text = if quoted_text.trim().is_empty()
        && evidence_type == etyma_engine_types::EvidenceType::ImageRegion
    {
    ...
    Some(etyma_engine_types::EvidenceIndexItemV1 {
        ...
        span: Some("page".into()),
        quoted_text,
        parse_confidence: etyma_engine_types::ContextPackParseConfidence::Unknown,
...
fn infer_evidence_type(page: &PageArtifact, raw_text: &str) -> etyma_engine_types::EvidenceType {
    if page
        .markdown_path
        .as_ref()
        .and_then(|path| fs::read_to_string(path).ok())
    ...
    {
        return etyma_engine_types::EvidenceType::Table;
    }

    if raw_text.trim().is_empty() && page.image_path.is_some() && page.error_message.is_none() {
        return etyma_engine_types::EvidenceType::ImageRegion;
    }
