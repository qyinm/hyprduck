...
pub(crate) fn answer_project(
    ...
    if question.is_empty() {
        return Ok(AnswerResponse {
            ...
            text: None,
            explanation: "Ask a concrete question before Etyma tries to answer from the graph."
                .into(),
            citations: Vec::new(),
...
pub(crate) fn apply_rename_correction(
    ...
    detail.description = format!(
        "Renamed from {} to {}. Etyma kept the previous canonical label as an alias so the evidence trail stays intact.",
...
pub(crate) fn apply_merge_correction(
...

    {
        ...
        target_detail.description = format!(
            "Merged {} into {}. Etyma kept all visible evidence on the surviving concept.",
...
pub(crate) fn apply_keep_separate_correction(
...

    {
        ...
        detail.description = format!(
            "Etyma kept the previous aliases under {} as distinct concept nodes after a manual correction.",
    ...
    for (index, alias) in source_detail.aliases.iter().enumerate() {
        ...
        project.details_by_node_id.insert(
            ...
            GraphNodeDetail {
                ...
                aliases: Vec::new(),
                description: format!(
                    "Created from a keep separate correction on {}. Etyma preserved the supporting evidence while treating this as its own concept.",
        ...
        replacements
    {
        ...
        project.details_by_node_id.insert(
            ...
            GraphNodeDetail {
                ...
                aliases,
                description: format!(
                    "Created from an explicit split correction on {}. Etyma moved the mapped evidence, edges, claims, and wiki references onto this replacement concept.",
                    source_detail.canonical_name
                ),
