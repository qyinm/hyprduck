...
pub(crate) fn answer_materialized_workspace_project(
    ...
    if question.is_empty() {
        return Ok(AnswerResponse {
            ...
            text: None,
            explanation: "Ask a concrete question before Etyma tries to answer from the materialized workspace graph."
                .into(),
            citations: Vec::new(),
    ...
    match status {
        ...
        AnswerStatus::LowConfidence => {
            if korean {
                ...
                Some(format_workspace_answer(
                    "Etyma found weak direct overlap, but these workspace notes are closest.",
                    &points,
                    &source_labels,
                    "Sources",
                ))
        ...
        AnswerStatus::Blocked => Some(format!(
            "Etyma cannot safely answer \"{}\" from the current materialized workspace yet.",
            question
        )),
        AnswerStatus::Stale => {
            Some("Etyma is still reading from a stale workspace snapshot.".into())
        }
    }
}
...
fn materialized_workspace_answer_explanation(
    ...
    match status {
        AnswerStatus::Grounded => format!(
            "Etyma answered \"{}\" from the materialized workspace graph with {} retrieved context item(s) and {} citation(s).{}",
            question,
            candidates.len(),
            citations.len(),
            selected_note
        ...
        AnswerStatus::LowConfidence => format!(
            "Etyma searched the materialized workspace graph for \"{}\", but the retrieved context had weak direct overlap.{}",
            question, selected_note
        ),
        AnswerStatus::Blocked => format!(
            "Etyma blocked this answer because it could not find materialized workspace context for \"{}\".{}",
            question, selected_note
        ),
        AnswerStatus::Stale => "Etyma is still reading from a stale workspace snapshot.".into(),
    }
}
    ...
    match detail.node.kind {
        GraphNodeKind::Source | GraphNodeKind::Document => {
            ...
            AnswerResponse {
                ...
                text: Some(format!(
                    "Etyma currently tracks {} concept nodes and {} explainable concept links in this workspace.",
...
pub(crate) fn answer_text_for_question(
    ...
    let evidence_summary = citations
        ...
        .unwrap_or_else(|| "Etyma could not find a directly relevant snippet yet.".into());
    let page_count = detail
        .evidence
        .iter()
        .map(|evidence| evidence.page_label.clone())
    ...
    match detail.node.kind {
        GraphNodeKind::Source | GraphNodeKind::Document => {
            ...
            match status {
                ...
                AnswerStatus::LowConfidence => format!(
                    "Etyma can partially answer \"{}\", but the graph only has weak snippet overlap. Closest visible support: {}",
                    question, evidence_summary
                ),
                AnswerStatus::Blocked | AnswerStatus::Stale => format!(
                    "Etyma cannot safely answer \"{}\" from the current workspace yet.",
                    question
                ),
            }
        }
        GraphNodeKind::Concept | GraphNodeKind::Page => match status {
            ...
            AnswerStatus::LowConfidence => format!(
                "Etyma found {} evidence refs for {}, but the question \"{}\" only weakly matches those snippets. Closest visible support: {}",
                detail.evidence.len(),
                detail.canonical_name,
                question,
                evidence_summary
            ...
            AnswerStatus::Blocked | AnswerStatus::Stale => format!(
                "Etyma cannot safely answer \"{}\" for {} yet.",
...
pub(crate) fn answer_explanation_for_question(
    ...
    match status {
        AnswerStatus::Grounded => format!(
            "Etyma answered \"{}\" using {} visible citation(s) attached to {}.",
            question,
            citations.len(),
            detail.canonical_name
        ),
        AnswerStatus::LowConfidence => format!(
            "Etyma kept this answer cautious because the question \"{}\" only loosely overlaps with the visible evidence on {}.",
            question, detail.canonical_name
        ),
        AnswerStatus::Blocked => format!(
            "Etyma blocked this answer because it could not find enough grounded evidence for \"{}\".",
            question
        ),
        AnswerStatus::Stale => "Etyma is still reading from a stale workspace snapshot.".into(),
    }
...
pub(crate) fn answer_suggested_actions(status: AnswerStatus) -> Vec<SuggestedAction> {
    match status {
        ...
        AnswerStatus::Blocked | AnswerStatus::Stale => vec![SuggestedAction {
            ...
            label: "Ask a narrower question".into(),
            description:
                "Etyma needs a more concrete, evidence-seeking question before it can answer."
                    .into(),
        }],
    }
}
...
pub(crate) fn edge_explanation(
    ...
    match edge.kind {
        RelationKind::SourceDocument => format!(
            "Etyma linked the source document to {} because this concept is grounded in cited snippets from the import.",
            target_label
        ),
        RelationKind::RelatedTo if edge.label == "Separated by correction" => format!(
            "Etyma keeps {} and {} separate because you explicitly split them during correction review.",
            source_label, target_label
        ),
        RelationKind::RelatedTo => format!(
            "Etyma linked {} and {} because they share {} visible evidence ref(s).",
            source_label,
            target_label,
            evidence.len()
        ),
