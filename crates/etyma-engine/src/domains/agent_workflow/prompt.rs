...
pub(crate) fn build_source_chunk_graph_prompt(
    ...
    Ok(format!(
        r#"You are Etyma's source-local graph candidate extraction agent.
...
- Return wikiPages as []; Etyma synthesizes wiki summaries after canonicalization.
- Do not emit source_of edges. Etyma will add canonical source_of edges after dedupe.
...
- Use stable raw nodeIds/relationIds/claimIds so repeated chunk runs remain readable. Etyma will assign canonical durable IDs later.
...
pub(crate) fn build_workspace_linking_prompt(
    ...
    Ok(format!(
        r#"You are Etyma's workspace linking agent.
...
- Return memories as [].
- Return wikiPages as []; Etyma synthesizes wiki pages after validated linking.
...
- Do not return source_of edges. Etyma owns source edges.
