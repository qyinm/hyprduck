...
pub(super) const KNOWLEDGE_DB_FILE_NAME: &str = "etyma.sqlite";
...
pub(crate) fn validate_graphqlite_gate(graph: &Graph) -> Result<GraphqliteGateReport> {
    graph
        .connection()
        .cypher("CREATE (n:EtymaGate {id: 'gate-load'})")
        .context("GraphQLite failed basic Cypher create")?;
    let result = graph
        .connection()
        .cypher("MATCH (n:EtymaGate {id: 'gate-load'}) RETURN n.id")
    ...
    graph
        .connection()
        .cypher("MATCH (n:EtymaGate {id: 'gate-load'}) DELETE n")
    ...
    graph
        .connection()
        .cypher("CREATE (n:EtymaGate {id: 'gate-rollback'})")
    ...
    let result = graph
        .connection()
        .cypher("MATCH (n:EtymaGate {id: 'gate-rollback'}) RETURN n.id")
