...
fn default_project_store_migrates_legacy_db_to_canonical_name() {
    ...
    let store_dir = temp.path().join("Etyma");
    ...
    assert_eq!(store.path, store_dir.join("etyma.sqlite"));
...
fn default_project_store_preserves_legacy_project_source_and_correction_rows() {
    let temp = tempfile::tempdir().expect("temp dir");
    let legacy_path = temp.path().join("Etyma").join("knowledge.sqlite3");
    ...
    assert_eq!(
        ...
        temp.path().join("Etyma").join("etyma.sqlite")
    );
    assert!(legacy_path.exists());
    assert_eq!(
        canonical_store
...
fn source_import_persists_canonical_db_rows_and_fts() {
    let temp = tempfile::tempdir().expect("temp dir");
    let store = KnowledgeProjectStore::new(temp.path().join("etyma.sqlite"));
...
fn source_import_redacts_page_artifact_paths() {
    let temp = tempfile::tempdir().expect("temp dir");
    let db_path = temp.path().join("etyma.sqlite");
...
fn latest_readable_marker_classifies_json_artifacts_as_migration_input() {
    ...
    assert_eq!(marker["canonicalStateStore"], "etyma.sqlite+graphqlite");
...
fn brain_reader_open_migrates_existing_materialized_json_into_sqlite() {
    ...
    .expect("write events");
    assert!(!workspace_root.join("etyma.sqlite").exists());
    ...
    let store = KnowledgeProjectStore::new(workspace_root.join("etyma.sqlite"));
    assert_eq!(count_table_rows(&store, "sources"), 1);
    assert_eq!(count_table_rows(&store, "source_pages"), 1);
...
fn brain_reader_migration_preserves_artifact_metadata() {
    ...
    fs::write(
        ...
        serde_json::json!({
            "schemaVersion": "etyma.context_pack.v1",
    ...
    artifact_metadata.evidence.insert(
        ...
        BTreeMap::from([(
            ...
            ContextPackEvidenceMetadataV0 {
                ...
                image_path: None,
                evidence_type: etyma_engine_types::EvidenceType::Text,
            },
        )]),
    );

    let external = etyma_engine_types::ContextPackV0::from_brain_context_pack(
        &pack,
        "ctx-alpha",
...
fn resolve_binary_prefers_path_before_common_locations() {
    ...
    let binary_path = bin_dir.join("etyma-test-bin");
    fs::write(&binary_path, "").expect("test bin");

    let old_path = std::env::var_os("PATH");
    std::env::set_var("PATH", &bin_dir);
    let resolved = resolve_binary("etyma-test-bin", &["/definitely/missing"]);
    match old_path {
