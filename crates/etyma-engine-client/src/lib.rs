...
pub trait EngineClient {
    ...
    fn get_context_pack(
        &self,
        request: etyma_engine_types::GetContextPackRequest,
    ...
    fn default() -> Self {
        Self {
            launch_spec: resolve_engine_launch()
                .unwrap_or_else(|_| EngineLaunchSpec::binary(PathBuf::from("etyma-engine"))),
        }
    }
}
...
impl EngineClient for SubprocessEngineClient {
    ...
    fn get_context_pack(
        ...
        let response = self.run_command::<
            etyma_engine_types::GetContextPackResponseData,
            etyma_engine_types::GetContextPackResponseData,
        >(
...
pub fn resolve_engine_bin() -> Result<PathBuf> {
    if let Some(explicit) = std::env::var_os("ETYMA_ENGINE_BIN") {
        return Ok(PathBuf::from(explicit));
    }

    if let Some(explicit) = std::env::var_os("CARGO_BIN_EXE_etyma-engine") {
        return Ok(PathBuf::from(explicit));
    }
    ...
    Err(anyhow!("unable to resolve etyma-engine binary"))
}

fn engine_binary_names() -> Vec<OsString> {
    if cfg!(target_os = "windows") {
        vec![
            OsString::from("etyma-engine.exe"),
            OsString::from(format!("etyma-engine-{}.exe", target_triple())),
        ]
    } else {
        vec![
            OsString::from("etyma-engine"),
            OsString::from(format!("etyma-engine-{}", target_triple())),
        ]
    }
}
...
pub fn resolve_engine_launch() -> Result<EngineLaunchSpec> {
    ...
    .context("unable to resolve etyma-engine runtime")?;
...
impl EngineLaunchSpec {
    ...
    fn cargo_run(workspace_root: PathBuf) -> Self {
        ...
        Self {
            ...
            display: "cargo run --quiet -p etyma-engine --".to_string(),
        }
    }
