...
fn default_golden_corpus_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../duckdocs-engine/tests/fixtures/brain-corpus")
}
...
fn run_mode(cases: &[GoldenCase], mode: GoldenEvalMode) -> Result<EvalCounts> {
    ...
    restore_env_var("DUCKDOCS_PROJECT_STORE", previous_store);
