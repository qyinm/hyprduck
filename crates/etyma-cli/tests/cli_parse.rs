...
use etyma_engine_types::{ContextPackV1, EvidenceIndexV1, EvidenceType, SourcePackV0};
...
fn doctor_reports_engine_resolution() {
    ...
    assert!(stdout.contains("Etyma CLI is available."));
}

#[test]
fn context_alias_writes_context_pack_v1() {
    let root = unique_temp_dir("etyma-cli-context-alias");
    ...
    let stdout = String::from_utf8(output.stdout).unwrap();
    assert!(stdout.contains("context-pack-v1: etyma.context_pack.v1"));
    assert!(stdout.contains("context-pack-v0: etyma.context_pack.v0"));
    ...
    assert_eq!(context_pack.schema_version, "etyma.context_pack.v1");
    let _ = fs::remove_dir_all(root);
}

#[test]
fn documents_search_alias_keeps_brain_search_compatibility() {
    let root = unique_temp_dir("etyma-cli-documents-search");
    fs::create_dir_all(root.join("default")).unwrap();
...
fn demo_writes_local_context_pack_artifacts() {
    let root = unique_temp_dir("etyma-cli-demo");
    ...
    assert_eq!(source_pack.schema_version, "etyma.source_pack.v0");
    ...
    assert_eq!(evidence_index.schema_version, "etyma.evidence_index.v1");
...
fn mcp_install_claude_code_writes_production_server_entry() {
    let home = unique_temp_dir("etyma-cli-mcp-install");
    ...
    assert_eq!(config["mcpServers"]["etyma"]["args"][0], "mcp");
    assert_eq!(config["mcpServers"]["etyma"]["args"][1], "serve");
    assert!(config["mcpServers"]["etyma"]["command"]
        .as_str()
        .unwrap()
        .contains("etyma"));
    assert!(home.join(".local/bin/etyma").exists());
...
fn mcp_install_codex_registers_server_and_shell_command() {
    let home = unique_temp_dir("etyma-cli-mcp-install-codex");
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_etyma"))
        ...
        .env("HOME", &home)
        .env("ETYMA_CODEX_BIN", &fake_codex)
        .env("ETYMA_MCP_ALLOWED_IMPORT_ROOTS", "/tmp/etyma-imports")
        .env("ETYMA_MCP_ALLOW_ROOT_DIR", "1")
        .env("ETYMA_MCP_ALLOWED_ROOTS", "/tmp/etyma-workspace")
        .env(
            "ETYMA_PROJECT_STORE",
            "/tmp/etyma-proof/knowledge.sqlite3",
        )
        .env("ETYMA_DISABLE_PROVIDER_GRAPH", "1")
        .output()
        .expect("mcp install command should run");
    ...
    assert!(calls.contains("mcp remove etyma"));
    assert!(calls.contains("mcp add --env"));
    assert!(calls.contains("etyma --"));
    assert!(calls.contains("--env ETYMA_MCP_ALLOWED_IMPORT_ROOTS=/tmp/etyma-imports"));
    assert!(calls.contains("--env ETYMA_MCP_ALLOW_ROOT_DIR=1"));
    assert!(calls.contains("--env ETYMA_MCP_ALLOWED_ROOTS=/tmp/etyma-workspace"));
    assert!(calls.contains("--env ETYMA_PROJECT_STORE=/tmp/etyma-proof/knowledge.sqlite3"));
    assert!(calls.contains("--env ETYMA_DISABLE_PROVIDER_GRAPH=1"));
    ...
    assert!(stdout.contains(
        "env: ETYMA_MCP_ALLOWED_IMPORT_ROOTS, ETYMA_MCP_ALLOW_ROOT_DIR, ETYMA_MCP_ALLOWED_ROOTS, ETYMA_PROJECT_STORE, ETYMA_DISABLE_PROVIDER_GRAPH"
    ));
    let _ = fs::remove_dir_all(home);
}
...
fn mcp_install_preserves_unmanaged_shell_symlink() {
    let home = unique_temp_dir("etyma-cli-mcp-install-preserve-symlink");
    let bin_dir = home.join(".local/bin");
    let foreign_target = home.join("custom-etyma");
    let shim_path = bin_dir.join("etyma");
...
fn mcp_install_replaces_previous_app_bundle_shell_symlink() {
    let home = unique_temp_dir("etyma-cli-mcp-install-managed-symlink");
    let bin_dir = home.join(".local/bin");
    let old_bundle_bin =
        home.join("Old Etyma.app/Contents/Resources/binaries/etyma-aarch64-apple-darwin");
    let shim_path = bin_dir.join("etyma");
    fs::create_dir_all(&bin_dir).unwrap();
...
fn brain_inspect_state_prints_selected_graph_state_and_related_events() {
    let root = unique_temp_dir("etyma-cli-graph-history");
...
fn brain_rollback_state_applies_selected_graph_state() {
    let root = unique_temp_dir("etyma-cli-graph-rollback");
    let workspace = root.join("default");
    fs::create_dir_all(workspace.join("events")).unwrap();
    fs::create_dir_all(workspace.join("graph")).unwrap();
    fs::write(
