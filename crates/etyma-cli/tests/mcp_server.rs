...
fn mcp_server_exposes_read_and_agent_session_write_brain_tools() {
    let root_dir = std::env::temp_dir().join(format!("etyma-mcp-empty-{}", std::process::id()));
    let root_dir_arg = root_dir.to_string_lossy().to_string();
    let _ = fs::remove_dir_all(&root_dir);
    fs::create_dir_all(&root_dir).expect("temp root");

    let mut child = Command::new(env!("CARGO_BIN_EXE_etyma"))
        .args(["mcp", "serve"])
        .env("ETYMA_PROJECT_STORE", root_dir.join("knowledge.sqlite3"))
        .env("ETYMA_MCP_ALLOW_ROOT_DIR", "1")
        .env("ETYMA_MCP_ALLOWED_ROOTS", &root_dir_arg)
        .env("ETYMA_MCP_ALLOW_LOCAL_PATHS", "1")
    ...
    write_message(
        ...
        json!({
            ...
            "params": {
                ...
                "capabilities": {},
                "clientInfo": { "name": "etyma-test", "version": "0.1.0" }
            }
        }),
    );
    let initialize = read_message(&mut reader);
    assert_eq!(initialize["result"]["serverInfo"]["name"], "etyma");
    ...
    let resource_uris = resources["result"]["resources"]
        ...
        .collect::<Vec<_>>();
    assert!(resource_uris.contains(&"etyma://brain/default/graph/snapshot"));
    assert!(resource_uris.contains(&"etyma://brain/default/wiki/index.md"));
    ...
    write_message(
        ...
        json!({
            ...
            "params": {
                "uri": format!("etyma://brain/default/graph/snapshot?rootDir={}", root_dir_arg)
            }
        }),
    );
    ...
    assert_eq!(
        ...
        "etyma://brain/default/graph/snapshot"
    );
    ...
    write_message(
        ...
        json!({
            ...
            "method": "resources/read",
            "params": {
                "uri": format!("etyma://brain/default/wiki/index.md?rootDir={}", root_dir_arg)
            }
        }),
    );
    ...
    assert_eq!(
        ...
        "etyma://brain/default/wiki/index.md"
    );
    assert_eq!(
    ...
    assert_eq!(
        snapshot_health["result"]["_meta"]["etymaGraphWikiCache"]["invalidated"], false,
        "{snapshot_health:#?}"
    );
    assert_eq!(
        snapshot_health["result"]["_meta"]["etymaGraphWikiCache"]["current"]["snapshotId"],
        "snapshot-mcp-readable"
    );
    assert_eq!(
        snapshot_health["result"]["_meta"]["etymaGraphWikiCache"]["current"]
        ...
        json!({
            ...
            "params": {
                ...
                "arguments": {
                    ...
                    "agentId": "codex",
                    "graphPatch": {
                        "schemaVersion": "etyma.graph_patch.v1",
    ...
    assert_eq!(
        graph_patch_tool["result"]["_meta"]["etymaGraphWikiCache"]["invalidated"], true,
        "{graph_patch_tool:#?}"
    );
    let graph_patch_cache =
        &graph_patch_tool["result"]["_meta"]["etymaGraphWikiCache"]["current"];
...
fn mcp_server_import_source_imports_allowlisted_markdown() {
    let root_dir = temp_test_dir("etyma-mcp-import-workspace");
    let import_root = temp_test_dir("etyma-mcp-import-source");
    let root_dir_arg = root_dir.to_string_lossy().to_string();
    ...
    let mut child = Command::new(env!("CARGO_BIN_EXE_etyma"))
        .args(["mcp", "serve"])
        .env("ETYMA_PROJECT_STORE", root_dir.join("knowledge.sqlite3"))
        .env("ETYMA_CONFIG_DIR", &config_dir)
        .env("ETYMA_MCP_ALLOW_ROOT_DIR", "1")
        .env("ETYMA_MCP_ALLOWED_ROOTS", &root_dir_arg)
        .env("ETYMA_MCP_ALLOWED_IMPORT_ROOTS", &import_root_arg)
        .env("ETYMA_DISABLE_PROVIDER_GRAPH", "1")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
    ...
    let mut restarted_child = Command::new(env!("CARGO_BIN_EXE_etyma"))
        .args(["mcp", "serve"])
        .env("ETYMA_PROJECT_STORE", root_dir.join("knowledge.sqlite3"))
        .env("ETYMA_CONFIG_DIR", &config_dir)
        .env("ETYMA_MCP_ALLOW_ROOT_DIR", "1")
...
fn mcp_server_import_source_rejects_source_path_outside_allowed_roots() {
    let root_dir = temp_test_dir("etyma-mcp-import-reject-workspace");
    let import_root = temp_test_dir("etyma-mcp-import-reject-source");
    let outside_root = temp_test_dir("etyma-mcp-import-reject-outside");
    let root_dir_arg = root_dir.to_string_lossy().to_string();
    let import_root_arg = import_root.to_string_lossy().to_string();
    ...
    assert!(text.contains("ETYMA_MCP_ALLOWED_IMPORT_ROOTS"));
    assert!(!text.contains(root_dir_arg.as_str()));
    assert!(!text.contains(import_root_arg.as_str()));
