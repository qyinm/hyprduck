...
pub(crate) fn policy_for_mcp_tool(tool_name: &str) -> ToolAutomationPolicy {
    let name = normalize_etyma_tool_name(tool_name);
    match name.as_deref() {
        Some("write_reject") => ToolAutomationPolicy {
            decision: AutomationDecision::RequiresApproval,
            destructive: true,
            reason: "rejecting or removing Etyma write state requires approval",
        },
        Some(tool) if AUTOMATIC_MCP_TOOLS.contains(&tool) => ToolAutomationPolicy {
            decision: AutomationDecision::Automatic,
            destructive: false,
            reason:
                "known Etyma action covered by existing MCP scope, evidence, and audit policy",
        },
        _ => ToolAutomationPolicy {
            decision: AutomationDecision::RequiresApproval,
            destructive: false,
            reason: "unknown Etyma action is not eligible for automatic execution",
        },
    }
}

pub(crate) fn normalize_etyma_tool_name(tool_name: &str) -> Option<String> {
    if tool_name.contains("__etyma__") {
        return tool_name
            .rsplit("__")
            .next()
...
mod tests {
    ...
    fn codex_mcp_tool_names_are_normalized() {
        let policy = policy_for_mcp_tool("mcp__etyma__read_health");
        assert_eq!(policy.decision, AutomationDecision::Automatic);
