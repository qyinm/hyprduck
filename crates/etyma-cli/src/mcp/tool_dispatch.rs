...
pub(in crate::mcp) const LOCAL_PATH_DISCLOSURE_ENV: &str = "ETYMA_MCP_ALLOW_LOCAL_PATHS";
...
pub(in crate::mcp) fn call_tool(
    ...
    let value = match name {
        ...
        _ => return Err(anyhow!("Unknown Etyma MCP tool: {name}")),
    };
...
pub(in crate::mcp) fn local_path_disclosure_for_tool(
    ...
    if !local_path_disclosure_allowed() {
        bail!("includeLocalPaths requires ETYMA_MCP_ALLOW_LOCAL_PATHS=1");
    }
    Ok(true)
}
