...
fn validate_import_source_path_rejects_file_outside_allowed_root() {
    ...
    assert!(error
        .to_string()
        .contains("ETYMA_MCP_ALLOWED_IMPORT_ROOTS"));
    clear_root_dir_env();
}
...
fn read_scope_rejects_root_dir_without_dev_env() {
    ...
    arguments.insert("rootDir".into(), Value::String("/tmp/etyma-test".into()));
...
fn read_scope_rejects_root_dir_without_allowed_roots() {
    ...
    assert!(error.to_string().contains("ETYMA_MCP_ALLOWED_ROOTS"));
    clear_root_dir_env();
}
...
fn resource_uri_rejects_root_dir_without_dev_env() {
    ...
    let error = parse_resource_uri("etyma://brain/default/wiki/index.md?rootDir=/tmp")
...
fn resource_uri_rejects_root_dir_when_dev_env_is_not_one() {
    ...
    clear_root_dir_env();
    std::env::set_var(ROOT_DIR_ENV, "0");
    let zero_error = parse_resource_uri("etyma://brain/default/wiki/index.md?rootDir=/tmp")
        .expect_err("rootDir=0 should stay disabled for resources");
    assert!(zero_error.to_string().contains("rootDir is disabled"));

    std::env::set_var(ROOT_DIR_ENV, "");
    let empty_error = parse_resource_uri("etyma://brain/default/wiki/index.md?rootDir=/tmp")
...
fn resource_uri_accepts_allowlisted_root_dir() {
    ...
    let temp = tempfile::tempdir().expect("temp dir");
    let uri = format!(
        "etyma://brain/default/wiki/index.md?rootDir={}",
        temp.path().display()
    );
