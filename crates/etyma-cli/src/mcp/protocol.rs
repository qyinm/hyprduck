...
fn initialize_result(message: &Value) -> Value {
    ...
    json!({
        ...
        "serverInfo": {
            "name": "etyma",
            "title": "Etyma Local Context",
            "version": env!("CARGO_PKG_VERSION")
        },
        "instructions": "Etyma exposes local, desktop-first, evidence-governed document context through read, search, context-pack, snapshot, and health tools. Use get_context_pack first, then search_documents or open cited sources, page evidence, wiki pages, nodes, or event history as needed."
    })
}
    ...
    let result = match call_tool(client, state, name, arguments, include_local_paths) {
        Ok(tool_result) => {
            ...
            if let Some(cache_state) = tool_result.cache_state {
                result["_meta"] = json!({
                    "etymaGraphWikiCache": cache_state
                });
            }
            result
        }
