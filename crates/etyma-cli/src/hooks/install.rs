...
const CONFIG_BLOCK_START: &str = "# BEGIN ETYMA HOOK AUTOMATION";
const CONFIG_BLOCK_END: &str = "# END ETYMA HOOK AUTOMATION";
const ETYMA_STATUS_PREFIX: &str = "Etyma:";
...
const CODEX_HOOK_SPECS: &[CodexHookSpec] = &[
    CodexHookSpec {
        ...
        status: "Etyma: loading context",
    },
    CodexHookSpec {
        event: "UserPromptSubmit",
        matcher: None,
        status: "Etyma: checking context",
    },
    CodexHookSpec {
        event: "PreToolUse",
        matcher: Some("mcp__etyma__.*|apply_patch|Edit|Write"),
        status: "Etyma: checking automation policy",
    },
    CodexHookSpec {
        event: "PermissionRequest",
        matcher: Some("mcp__etyma__.*"),
        status: "Etyma: checking MCP approval policy",
    },
    CodexHookSpec {
        event: "PostToolUse",
        matcher: Some("mcp__etyma__.*|apply_patch|Edit|Write"),
        status: "Etyma: recording workflow guidance",
    },
    CodexHookSpec {
        event: "Stop",
        matcher: None,
        status: "Etyma: finalizing hook state",
    },
];
...
impl CodexApprovalSpec {
    fn toml_line(self) -> String {
        format!(
            "mcp_servers.etyma.tools.{}.approval_mode = \"{}\"",
...
fn merge_hooks_json(path: &Path, command: &str) -> Result<()> {
    ...
    for spec in CODEX_HOOK_SPECS {
        replace_etyma_group(hooks, spec, codex_hook_group(*spec, command))?;
    }
...
fn replace_etyma_group(
    ...
    array.retain(|entry| !is_etyma_managed_group(entry));
    array.push(group);
    Ok(())
}
            ...
            CODEX_HOOK_SPECS.iter().all(|spec| {
                hooks
                    ...
                    .is_some_and(|array| {
                        array
                            .iter()
                            .any(|entry| is_etyma_managed_group_for_spec(entry, spec))
                    })
            })
        });
...
fn managed_config_block_span(text: &str) -> Result<Option<ManagedConfigBlockSpan>> {
    ...
    let Some(relative_end) = text[content_start..].find(CONFIG_BLOCK_END) else {
        return Err(anyhow!(
            "Etyma managed config block is missing its end marker"
        ));
    };
...
fn is_etyma_managed_group(entry: &Value) -> bool {
    entry
        .get("hooks")
        .and_then(Value::as_array)
        .is_some_and(|hooks| hooks.iter().any(is_etyma_managed_hook))
}

fn is_etyma_managed_group_for_spec(entry: &Value, spec: &CodexHookSpec) -> bool {
    ...
    matcher_matches
        && entry
            ...
            .is_some_and(|hooks| {
                hooks
                    .iter()
                    .any(|hook| is_etyma_managed_hook_for_spec(hook, spec))
            })
}

fn is_etyma_managed_hook(hook: &Value) -> bool {
    ...
    let status_is_managed = hook
        .get("statusMessage")
        .and_then(Value::as_str)
        .is_some_and(|status| status.starts_with(ETYMA_STATUS_PREFIX));
    command_is_managed || status_is_managed
}

fn is_etyma_managed_hook_for_spec(hook: &Value, spec: &CodexHookSpec) -> bool {
    hook.get("command")
        .and_then(Value::as_str)
        .is_some_and(|command| {
            command.ends_with("hooks run codex") || command.contains(" hooks run codex")
...
fn codex_mcp_approval_block() -> String {
    ...
    block.push_str("# Etyma hooks keep destructive removal/overwrite actions approval-gated.\n");
...
fn write_file_atomically(path: &Path, content: String) -> Result<()> {
    ...
    let file_name = path
        ...
        .unwrap_or("config");
    let temporary = path.with_file_name(format!(".{file_name}.etyma.tmp"));
