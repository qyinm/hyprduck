...
pub fn render(frame: &mut Frame, app: &App) {
    ...
    let tabs = Tabs::new(titles)
        .block(Block::default().borders(Borders::ALL).title("Etyma"))
        .highlight_style(
            Style::default()
                .fg(ACCENT_CYAN)
...
fn render_home(frame: &mut Frame, area: ratatui::layout::Rect) {
    ...
    let tips = Paragraph::new(vec![
        ...
        Line::from(""),
        Line::from(vec![
            Span::styled("etyma parse", Style::default().fg(ACCENT_GREEN)),
    ...
    let overview = Paragraph::new(
        "Etyma TUI orchestrates parse jobs against an external engine.\n\n\
         Start here:\n\
         - parse a file\n\
         - inspect recent jobs\n\
         - configure engines",
