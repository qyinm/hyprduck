...
use etyma_engine_client::{resolve_engine_launch, EngineClient, SubprocessEngineClient};
...
const DEMO_MARKDOWN: &str = r#"# Etyma Demo Contract
...
fn install_claude_code_mcp() -> Result<()> {
    ...
    config["mcpServers"]["etyma"] = serde_json::json!({
    ...
    println!("Installed Etyma MCP for Claude Code.");
...
fn install_codex_mcp() -> Result<()> {
    ...
    let codex_bin = std::env::var_os("ETYMA_CODEX_BIN").unwrap_or_else(|| "codex".into());
    let _ = std::process::Command::new(&codex_bin)
        .args(["mcp", "remove", "etyma"])
        .status();
    ...
    let status = command
        .args(["etyma", "--", &current_exe, "mcp", "serve"])
        .status()
    ...
    println!("Installed Etyma MCP for Codex.");
    println!("command: {current_exe} mcp serve");
...
fn codex_mcp_forwarded_env_names() -> [&'static str; 5] {
    [
        "ETYMA_MCP_ALLOWED_IMPORT_ROOTS",
        "ETYMA_MCP_ALLOW_ROOT_DIR",
        "ETYMA_MCP_ALLOWED_ROOTS",
        "ETYMA_PROJECT_STORE",
        "ETYMA_DISABLE_PROVIDER_GRAPH",
    ]
}
...
fn current_exe_string() -> Result<String> {
    let current_exe = std::env::current_exe()
        .map_err(|error| anyhow::anyhow!("failed to locate etyma binary: {error}"))?;
...
fn install_shell_shim(home: &std::path::Path, current_exe: &str) -> Result<()> {
    ...
    match std::fs::symlink_metadata(&shim_path) {
        Ok(metadata) if metadata.file_type().is_symlink() => {
            ...
            if !is_managed_etyma_cli_target(&resolved_target) {
                eprintln!(
...
fn is_managed_etyma_cli_target(target: &std::path::Path) -> bool {
    let Some(file_name) = target.file_name().and_then(|value| value.to_str()) else {
        return false;
    };
    let target = target.to_string_lossy();
    file_name.starts_with("etyma-") && target.contains(".app/Contents/Resources/binaries/")
}
...
fn run_doctor() -> Result<()> {
    println!("Etyma CLI is available.");
...
fn run_demo(command: cli::DemoCommand) -> Result<()> {
    ...
    let previous_output_dir = std::env::var_os("ETYMA_OUTPUT_DIR");
    let previous_provider_graph = std::env::var_os("ETYMA_DISABLE_PROVIDER_GRAPH");
    std::env::set_var("ETYMA_PROJECT_STORE", root.join("knowledge.sqlite3"));
    std::env::set_var("ETYMA_OUTPUT_DIR", &root);
    std::env::set_var("ETYMA_DISABLE_PROVIDER_GRAPH", "1");

    let result = run_demo_with_root(&root, &fixture_path, &command.query);

    restore_env_var("ETYMA_PROJECT_STORE", previous_store);
    restore_env_var("ETYMA_OUTPUT_DIR", previous_output_dir);
    restore_env_var("ETYMA_DISABLE_PROVIDER_GRAPH", previous_provider_graph);
    ...
    println!(
        "next: etyma context --root {} --workspace {DEMO_WORKSPACE_ID} --write-context-pack \"{}\"",
        root.display(),
...
fn write_demo_source_artifacts(root: &Path, fixture_path: &Path) -> Result<SourceArtifactManifest> {
    ...
    let markdown_path = artifact_root.join("etyma-demo.md");
    ...
    let manifest = SourceArtifactManifest {
        ...
        output_name: "etyma-demo".into(),
        status: IngestStatus::Ingested,
        description: "Built-in Etyma demo fixture.".into(),
...
fn unique_demo_root() -> Result<PathBuf> {
    ...
    Ok(std::env::temp_dir().join(format!("etyma-demo-{nanos}")))
}
...
fn print_graph_history(
    response: etyma_engine_types::ReadGraphHistoryResponseData,
) -> Result<()> {
...
fn print_graph_state_inspection(
    ...
    events: etyma_engine_types::ReadRecentEventsResponseData,
) -> Result<()> {
...
fn related_events_request_for_state(
    request: &etyma_engine_types::ReadGraphHistoryRequest,
    state: &GraphHistoryEntry,
...
fn print_event_history(
    response: etyma_engine_types::ReadRecentEventsResponseData,
) -> Result<()> {
