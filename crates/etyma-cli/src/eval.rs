...
use etyma_engine_client::{EngineClient, SubprocessEngineClient};
...
fn default_golden_corpus_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../etyma-engine/tests/fixtures/brain-corpus")
}
...
fn run_mode(cases: &[GoldenCase], mode: GoldenEvalMode) -> Result<EvalCounts> {
    ...
    std::env::set_var("ETYMA_OUTPUT_DIR", &eval_root);
...
fn page_refs_from_evidence(
    evidence_refs: &[etyma_engine_types::EvidenceRef],
) -> Vec<etyma_engine_types::StructuredExtractionPageRef> {
    let mut refs = BTreeMap::<(String, Option<usize>), _>::new();
    for evidence in evidence_refs {
        refs.entry((evidence.page_label.clone(), evidence.page_index))
            .or_insert_with(|| etyma_engine_types::StructuredExtractionPageRef {
...
fn unique_eval_root(mode: GoldenEvalMode) -> Result<PathBuf> {
    let timestamp = SystemTime::now()
        ...
        .as_nanos();
    Ok(std::env::temp_dir().join(format!("etyma-golden-{}-{timestamp}", mode.label())))
}
