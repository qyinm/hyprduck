...
- `tools/list` includes `import_source`, `get_context_pack`, `search_documents`,
  `read_source`, and `read_page_evidence`;
- if MCP import is part of the proof, `HYPRDUCK_MCP_ALLOWED_IMPORT_ROOTS` is set
  and `import_source` returns a `sourceId`, Source Pack schema, Evidence Index
  schema, and nonzero evidence count without leaking local paths;
...
| `parsing` | No usable source artifacts exist for the workspace. | Add a PDF, DOCX, DOC, Markdown, or image file and wait for import to finish. |
