...
severity: high
applies_when:
  - "Agents need to create graph/wiki state from Etyma evidence without Etyma owning the model call."
...
Provider graph generation remains useful when Etyma should call OpenRouter or Ollama during import and graph retry. It is the wrong boundary when an external coding agent has already read the source evidence through MCP and should use its own model runtime to produce graph/wiki changes.

The resolved pattern is `graph_patch_apply`: a controlled mutating MCP tool that accepts an agent-generated `etyma.graph_patch.v1` payload, validates it against existing source/evidence records, materializes it through the canonical graph snapshot path, and records an auditable brain event. Etyma owns the storage contract; the calling agent owns the model call.
...
"graph_patch_apply" => {
    let graph_patch: GraphPatch = serde_json::from_value(graph_patch_value)
        .context("argument graphPatch does not match Etyma graph patch schema")?;
...
- `schemaVersion` must match `etyma.graph_patch.v1`.
...
This boundary keeps Etyma from becoming a generic model router while still making the graph canvas and graph/wiki read model useful for agent-generated knowledge. Agents can use Codex, Claude Code, Pi, or another runtime to reason over retrieved evidence, then hand Etyma a small, typed patch.
...
Keeping the patch schema in `etyma-engine-types` also prevents MCP schema drift. The JSON schema exposed by `tools/list` needs tests that cover every engine contract field so an agent can construct valid patches without reverse-engineering Rust internals.
...
3. Construct a `etyma.graph_patch.v1` payload.
...
```bash
cargo test -p etyma-engine -p etyma-engine-client -p etyma-cli -p etyma-engine-types
cargo test -p etyma-engine-types brain_api_requests_round_trip
cargo test -p etyma-cli graph_patch_mcp_schema_covers_engine_contract_fields
cargo test -p etyma-engine agent_graph_patch
cargo test -p etyma-cli --test mcp_server mcp_server_exposes_read_and_agent_session_write_brain_tools
git diff --check
```
